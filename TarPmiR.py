...
Original code can be found here: http://hulab.ucf.edu/research/projects/miRNA/TarPmiR
"""
...
class Interactions:
	...
	def energy_site(self):
		...
		pp1=subprocess.Popen([miranda,intID+"_mir_seq.txt",intID+"_m_seq.txt","-sc", "60","-en","1"],stdout=subprocess.PIPE, stdin=subprocess.PIPE,stderr=subprocess.PIPE)
		#out,err=pp1.communicate(input=ip_seq)
		...
		binding_str = None
		for item in out:
			if "Query" in item and "Filename" not in item:
				query = item
			elif "Ref" in item and "Filename" not in item:
				ref = item
				if query and binding_str:
					seed_match.append([query,binding_str,ref])
					query = None
					ref = None
					binding_str = None
			elif '|' in item or ":" in item:
				binding_str = item
		# seed_match = list(divide_chunks(seed_match,3))
		out=[item for item in out if len(item)>1 and item[0]=='>' and item[1]!=">"]
		out=[item.split("\t") for item in out]
		out1=out
		out=[",".join(item[5].split(" ")) for item in out if len(item)>6]
		self.mirloc = out
		self.mrloc = [",".join(item[6].split(" ")) for item in out1 if len(item)>6]
		self.seed_match_string = seed_match
...
class binding:
	...
	def AU_content(self):
		...
		A=[0,0,0,0]
		seqt=self.m.seq[seed_pos[0]:seed_pos[1]].upper()
		dN={"A":0,"C":1,"G":2,"T":3}
		for i in seqt:
			if i in dN:
				A[dN[i]]+=1
...
def main():
	...
	for i in range(len(mir_info)):
		mir_info[i][1] = mir_info[i][1].upper().replace('U','T')
	# read in mRNA
	mRNA_file=File(mrna_path)
	mRNA_seq=mRNA_file.readSeq()
	for i in range(len(mRNA_seq)):
		mRNA_seq[i][1] = mRNA_seq[i][1].upper().replace('U','T')
	...
	for i in mRNA_seq:
		try:
			subprocess.Popen("rm core.*", shell=True)
		except Exception:
			pass
		...
		for j in mir_info:
			...
			for i in range(len(bs)):
				if len(ii.mrloc[i].split(","))>1 and ii.mrloc[i].split(",")[0] !='' and ii.mrloc[i].split(",")[1] !='':
					k = bs[i]
					bijk=binding(mir,m,k)
					if bijk.en is not None:
						F=bijk.getFeatures()
						FF=np.array(F).reshape(1,-1)
						pb=RR.predict_proba(FF)[0][1]
						out_bs=[]
						out_bs.append([bijk.mir.name,bijk.m.name]+[k]+[ii.mrloc[i]]+[str(pb)]+[item for item in ii.seed_match_string[i]] +[str(item) for item in F])
						out_bs=['\t'.join(item) for item in out_bs]
						out_bs='\n'.join(out_bs)
						f.write(out_bs)
						f.write("\n")
						f.flush()
